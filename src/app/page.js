    ...
    if (isEdit) {
      const updatedNotes = notes.map(note => {
        if (note.id === selectedId) {
          // Tetap gunakan properti lama (warna/rotate), hanya ubah konten
              ...
              >
                ...
                <div className={`relative bg-white p-2.5 rounded-[1.5rem] shadow-2xl shadow-gray-200/50 border border-gray-50 flex-1 max-w-[340px] transition-transform hover:scale-[1.02] duration-300 z-10 ${note.rotate}`}>
                  ...
                  <div className={`p-6 sm:p-8 rounded-[1rem] ${theme.bg[note.color]} h-full flex flex-col justify-start`}>
                    ...
                    <button 
                      onClick={(e) => {e.preventDefault(), handleEditNote(note)}}
                      className="mt-5 bg-gradient-to-r from-[#ff6823] to-[#e44600] hover:from-[#f4550e] hover:to-[#d04000] text-white px-10 py-4 rounded-[1.5rem] font-bold shadow-lg shadow-[#ff6823]/30 transition-all active:scale-95 flex items-center gap-3"
                    >
                      <span className="text-xl">Edit</span> 
                    </button>
                    <button 
                      className="mt-3 text-black px-10 py-4 rounded-[1.5rem] font-bold shadow-lg shadow-[#ff6823]/30 transition-all active:scale-95 flex items-center gap-3"
                      onClick={() => handleDeleteNote(note.id)}
                    >
                      <span className="text-xl">Hapus</span> 
                    </button>
                  </div>
                </div>
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4 transition-opacity">
          <div className="bg-white p-8 rounded-[2rem] shadow-2xl max-w-md w-full mx-4 border border-gray-100">
            <h2 className="text-2xl font-extrabold mb-6 text-gray-800 text-center tracking-tight">{isEdit ? "Edit Catatan" : "Tambah Note"}</h2>
